          ...
          species.name = NA, saveGIF = TRUE, fpath ="growth_gifs",  fname = "GIF_plotRings", species, treeid) 
{
  ...
  p <- ggplot(circle.data.df, aes(x,y, group = year)) + geom_path(color = "bisque1")+coord_equal()+
    theme_bw(base_size = 16)+   theme(panel.grid = element_blank(), axis.text.y = element_blank(), axis.ticks = element_blank(), axis.title.y = element_blank(), axis.line = element_blank(), 
  ...
  TRW$year <-as.integer(rownames(TRW))
  p2 <- ggplot(TRW, aes(x = year, y = trwN)) + geom_line(color = "bisque1", size = 2)+
    theme_bw(base_size = 16)+   theme(panel.grid = element_blank(), axis.ticks = element_blank(), 
                                      panel.background = element_rect(fill = "transparent", colour = NA),  plot.background = element_rect(fill = "transparent", colour = NA), 
                                      axis.line = element_line(size = 0.5, linetype = "solid", colour = "bisque1"))+
    #transition_layers(year, layer_length = 1, transition_length = 2, keep_layers = TRUE, from_blank = TRUE) 
    transition_reveal(along = year)+
    enter_appear()+labs(title = 'Year: {frame_along}', y = paste(species, treeid ,"\n width in (millimeters)"))
  p2
  
 animate(p, fps = 4, width = 650, height = 650,bg = 'transparent')
  anim_save("a.gif")
 
animate(p2, fps = 4, width = 850, height = 650, bg = 'transparent')
  anim_save("b.gif")
  ...
  #anim_save(paste0(fname, "_tree.gif"))
  magick::image_write(new_gif, path=paste0(fname, "_tree.gif"), density = 350)
}
